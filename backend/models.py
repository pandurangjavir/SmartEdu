...
class User(db.Model):
    ...
    def set_password(self, cleartext_password):
        self.password = generate_password_hash(cleartext_password)
        
    def check_password(self, cleartext_password):
        return check_password_hash(self.password, cleartext_password)
...
class Subject(db.Model):
    ...
    total_classes = db.Column(db.Integer, default=50)
    description = db.Column(db.Text)
    is_active = db.Column(db.Boolean, default=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class Student(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint('roll_no', 'class_id', name='uq_student_roll_class'),)
    
    student_id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.user_id'), unique=True, nullable=False)
    roll_no = db.Column(db.String(20), nullable=False)  # unique per class, not globally
    class_id = db.Column(db.Integer, db.ForeignKey('classes.class_id'), nullable=False)
    ...
    def to_dict(self):
        ...
        try:
            if self.user:
                ...
                data['user'] = {'name': self.user.name, 'email': self.user.email}
        except Exception:
            pass
            
        try:
            if self.class_:
                data['class_name'] = self.class_.class_name
        except Exception:
            pass
...
class Fine(db.Model):
    ...
    status = db.Column(db.Enum('Pending', 'Pending Approval', 'Approved'), default='Pending')
    receipt_data = db.Column(db.Text(16777215)) # Base64 string for receipts (MediumText)
    issued_date = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
    
    # Relationship back to student
    student = db.relationship('Student', backref='fines_list', lazy=True)
