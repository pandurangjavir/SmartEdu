...
const AdminStudentEditor = ({ studentId, onClose, initialTab = 'marks' }) => {
  ...
  const handleAttendanceUpdate = async () => {
    ...
    try {
      ...
      const response = await axios.put(`/api/admin/students/${studentId}/attendance`, {
        attendance: studentData.attendance
      });
      ...
      // Refresh the student data to show updated values
  ...
  const updateAttendance = (index, field, value) => {
    ...
    // Recalculate attendance percentage when present_count or total_classes changes
    if (field === 'present_count' || field === 'total_classes') {
      const present = updatedAttendance[index].present_count || 0;
      const total = updatedAttendance[index].total_classes || 1;
      updatedAttendance[index].attendance_percentage = total > 0 ? (present / total) * 100 : 0;
    }
          ...
          {activeTab === 'marks' && (
            <div>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium">Academic Marks</h3>
                <button
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {studentData.marks.map((mark, index) => (
                      <tr key={mark.subject_id}>
                        <td className="px-4 py-3 text-sm font-semibold text-gray-900">{mark.subject_name}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              className={`w-24 px-3 py-1.5 border rounded-lg text-sm font-bold focus:ring-2 focus:ring-blue-500 transition-colors ${mark.obtained_marks > mark.total_marks ? 'border-red-500 bg-red-50 text-red-900' : 'border-gray-300'}`}
                            />
                            <span className="text-gray-500 font-bold">/ {mark.total_marks}</span>
                          </div>
                          {mark.obtained_marks > mark.total_marks && (
                            <div className="text-[10px] text-red-600 font-bold mt-1 uppercase tracking-tighter">
                              ⚠️ Exceeds max marks
                            </div>
                          )}
                        </td>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {studentData.attendance.map((att, index) => (
                      <tr key={att.subject_id}>
                        <td className="px-4 py-3 text-sm font-semibold text-gray-900">{att.subject_name}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <input
                              type="number"
                              min="0"
                              max={att.total_classes}
                              value={att.present_count}
                              onChange={(e) => updateAttendance(index, 'present_count', parseInt(e.target.value) || 0)}
                              className={`w-24 px-3 py-1.5 border rounded-lg text-sm font-bold focus:ring-2 focus:ring-blue-500 transition-colors ${att.present_count > att.total_classes ? 'border-red-500 bg-red-50 text-red-900' : 'border-gray-300'}`}
                            />
                            <span className="text-gray-500 font-bold">/ {att.total_classes}</span>
                          </div>
                          {att.present_count > att.total_classes && (
                            <div className="text-[10px] text-red-600 font-bold mt-1 uppercase tracking-tighter">
                              ⚠️ Exceeds total classes
                            </div>
                          )}
                        </td>
                        <td className="px-4 py-3 text-sm text-gray-500 italic">
                        ...
                        <td className="px-4 py-3">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold ${
                            att.attendance_percentage < 75 ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'
                          }`}>
                            {att.attendance_percentage ? Number(att.attendance_percentage).toFixed(1) : '0.0'}%
                          </span>
                        </td>
                      </tr>
                    ))}
